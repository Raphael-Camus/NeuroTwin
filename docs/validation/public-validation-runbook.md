...
| 2 | HCP task-fMRI transfer check | Human Connectome Project | Check whether the surrogate and acquisition policy remain stable on HCP task contrasts. | Subject-split metrics and behavior-alignment signals are reproducible across tasks. |
| 3 | ABCD cohort stress test | ABCD Study | Use large-cohort developmental MRI only when approved access and governance are clear. | Generalization, site effects and QC gates are reported separately from model score. |
...
| Cognitive control | 0.88 | Advance as dry-run with explicit review gates | OpenNeuro BIDS smoke test |
| Closed-loop neuro experiment | 0.88 | Advance as dry-run with explicit review gates | OpenNeuro BIDS smoke test |
...
5. Complete `public_validation_protocol_template.md` before interpreting model scores.
6. Update `validation_ledger`, `public_validation_manifest.json` and the acquisition portfolio.
